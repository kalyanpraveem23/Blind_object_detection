...
def estimate_distance(w):
    constant_ratio = 20
    return constant_ratio / w * 10  # Multiply distance by 10 to adjust to centimeters

# Create a queue for TTS messages
tts_queue = queue.Queue()

# Function to speak TTS messages
def speak_messages():
    while True:
...
# Start TTS message processing thread
tts_thread = threading.Thread(target=speak_messages)
...
def obstacle_detection():
    ...
    while True:
        ...
        for class_id, bbox in zip(class_ids, bboxes):
            ...
            cv2.putText(frame, f"{class_name} - Distance: {distance:.2f} centimeters", (x, y - 10), cv2.FONT_HERSHEY_PLAIN, 2, (200, 0, 50), 2)
            cv2.rectangle(frame, (x, y), (x + w, y + h), (200, 0, 50), 3)

            tts_queue.put(f"Detected {class_name} at {distance:.2f} centimeters")
